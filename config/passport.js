...
passport.use(
  new OpenIDStrategy(
    ...
    (req, identifier, done) => {
      ...
      const profileURL = `http://api.steampowered.com/ISteamUser/GetPlayerSummaries/v0002/?key=${
      ...
      if (req.user) {
        User.findOne({ steam: steamId }, (err, existingUser) => {
          ...
          if (existingUser) {
            req.flash('errors', {
              msg:
                'There is already an account associated with the SteamID. Sign in with that account or delete it, then link it with your current account.'
            });
            done(err);
          } else {
            User.findById(req.user.id, (err, user) => {
